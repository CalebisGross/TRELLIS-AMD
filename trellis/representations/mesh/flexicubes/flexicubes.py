...
def check_tensor(tensor, shape, throw=True):
    """
    Simple replacement for kaolin.utils.testing.check_tensor.
    Validates that `tensor` matches the expected `shape`.
    Shape elements can be None to represent variable dimensions.
    """
    ...
    if len(tensor.shape) != len(shape):
        if throw:
            raise ValueError(f"Expected tensor with {len(shape)} dims, got {len(tensor.shape)}")
        return False
    for actual, expected in zip(tensor.shape, shape):
        if expected is not None and actual != expected:
            if throw:
                raise ValueError(f"Shape mismatch: expected {shape}, got {tensor.shape}")
            return False
    return True

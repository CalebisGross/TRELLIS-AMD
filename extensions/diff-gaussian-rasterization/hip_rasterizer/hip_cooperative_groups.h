...
public:
  __device__ __forceinline__ unsigned long long thread_rank() const {
    // Calculate linear thread index for any grid/block configuration
    // Block linear index
    unsigned long long block_idx =
        blockIdx.x + (unsigned long long)blockIdx.y * gridDim.x +
    ...
    // Thread linear index within block
    unsigned long long thread_idx = threadIdx.x + threadIdx.y * blockDim.x +
                                    threadIdx.z * blockDim.x * blockDim.y;

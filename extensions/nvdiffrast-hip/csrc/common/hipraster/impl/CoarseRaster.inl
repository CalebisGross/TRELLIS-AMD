...
__device__ __inline__ void coarseRasterImpl(const CRParams& p, char* s_smem) {
  return; // SAFE-MODE: real impl crashes the GPU on RDNA3 with current toolchain.
          // See CLAUDE.md "Active Investigation". Remove this line to re-test.
  // Diagnostic checkpoint markers + Test 42 OOB checks remain below for future debugging.
